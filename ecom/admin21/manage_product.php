...
foreach($multipleImageArr as $list){
	...
	echo '<input type="hidden" name="product_images_id[]" value="'.$list['id'].'"/></div>';
								...
								<div class="form-group">
									...
									<textarea name="description" id="para6" placeholder="Enter product description" class="form-control" required><?php echo $description?></textarea>
									 <!-- Paragraph 6 -->
...
<script>
    ClassicEditor
        .create(document.querySelector('#para6'))
        .catch(error => {
            console.error(error);
        });
</script>
